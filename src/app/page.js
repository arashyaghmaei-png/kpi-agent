...
Bekannte Warnsignale: KW20 schlechteste Woche (NPS 26, Termintreue 85,7%). KW23-24 kritischer Abfall CC 70%.

Du erhältst echte Techniker-Daten aus dem Telekom Auftragsinfo-Export. Bewerte jeden Techniker, gib Frühwarnungen bei >=7% Abweichung unter Baseline, und formuliere konkrete Leitstellen-Empfehlungen.
...
## Frühwarnungen
[Nur kritische Fälle mit Name und konkretem Problem]
  ...
  return (
    <div style={{ marginBottom: 6 }}>
      ...
      <div style={{ background: "#1f2937", borderRadius: 2, height: 6, position: "relative" }}>
        <div style={{ width: `${Math.min(100, value)}%`, background: color, height: "100%", borderRadius: 2, transition: "width 0.6s ease" }} />
...
function TechCard({ tech }) {
  ...
  const worst = [ccStatus, ttStatus, lqStatus].includes("kritisch") ? "kritisch"
    : [ccStatus, ttStatus, lqStatus].includes("warnung") ? "warnung" : "gut";
  ...
  return (
    <div style={{ background: "#111827", border: `1px solid ${borderColor}`, borderRadius: 8, padding: "16px 18px", marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: "#6b7280", marginTop: 2 }}>
            OD {tech["OD"]} · {tech["Anzahl"]} Aufträge · {tech["Sterne"]} Sterne
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 8, fontSize: 11, color: "#6b7280" }}>
        ...
        <span>Geplatzte T.: <span style={{ color: parsePercent(tech["T. Geplatz"]) > 5 ? "#f87171" : "#4ade80" }}>{tech["T. Geplatz"]}</span></span>
      </div>
    </div>
  );
}
...
export default function KPIAgent() {
  ...
  const runAnalysis = async () => {
    ...
    const dataStr = techniker.map(t =>
      `${t["Name"]} (OD${t["OD"]}): CC=${t["CC"]}, Termintreue=${t["Termintreue"]}, Lösungsquote=${t["Erledigt B"]}, Infoquote=${t["Infoquote P"]}, NPS=${t["NPS PB"]}, Aufträge=${t["Anzahl"]}, Sterne=${t["Sterne"]}, Geplatzte=${t["T. Geplatz"]}`
    ).join("\n");
    try {
      const res = await fetch("/api/analyse", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: `Analysiere diese Techniker-KPIs (Telekom Auftragsinfo):\n\n${dataStr}` }],
        }),
      });
  ...
  const criticalCount = techniker.filter(t => {
    const cc = parsePercent(t["CC"]);
    const tt = parsePercent(t["Termintreue"]);
    return [getStatusVal(cc, BASELINE.cc_rate), getStatusVal(tt, BASELINE.termintreue)].includes("kritisch");
  }).length;
    ...
    <div style={{ background: "#0a0e1a", minHeight: "100vh", fontFamily: "system-ui, sans-serif", color: "#e5e7eb" }}>
      <div style={{ borderBottom: "1px solid #1f2937", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 12, color: "#6b7280" }}>Techniker-Kontrolle · Auftragsinfo</span>
        </div>
      ...
      <div ref={dashboardRef} style={{ maxWidth: 760, margin: "0 auto", padding: "24px 20px" }}>
        {!techniker.length && (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            ...
            <div style={{ fontSize: 13, color: "#6b7280", marginBottom: 24 }}>Telekom Auftragsinfo → Auswertungen → SMS-Feedback → CSV-Export</div>
