...
export default function LoginPage() {
  ...
  // Firmenname aus derselben Einstellung wie der "Firma"-Knopf im Agenten
  const [firmaLabel, setFirmaLabel] = useState("Leitstelle");
  useEffect(() => {
    const f = localStorage.getItem("firma_name");
    if (f && f.trim()) setFirmaLabel(f.trim() + " - Leitstelle");
  }, []);
    ...
    <div style={{ minHeight: "100vh", background: "#0a0f1a", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 16, padding: "40px 36px", width: 360 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 12, color: "#6b7280", marginTop: 4 }}>{firmaLabel}</div>
        </div>
